...
public class Form {
    ...
    public String getForm(HashMap attributes) {
        String myForm = form;
        ...
        while (it.hasNext()) {
            ...
            if (elemAttributes.containsKey(providedAttribKey)) {
                ...
                for (int line = 0; line < elemAttributes.get(providedAttribKey).size(); line++) {
                    ...
                    if (fixedlength > 0) {
                        if (pad == LEFT) {
                            replacement = Tools.padL(replacement, fixedlength, fillchar);
                        } else if (pad == CENTER){
                            replacement = Tools.padC(replacement, fixedlength, fillchar);
                        } else {
                            replacement = Tools.padR(replacement, fixedlength, fillchar);
                        }
                    }
                    ...
                    myForm = Tools.replace(myForm, "$" + providedAttribKey + (line + 1) + "$", replacement);
                }
