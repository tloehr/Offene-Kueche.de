...
public class PrintProcessor extends Thread {
    ...
    public PrintProcessor(JProgressBar pb) {
        super();
    ...
    public void run() {
        while (!interrupted) {
            try {
                if (!printQueue.isEmpty()) {
                    ...
                    for (int i = 0; i < size; i++) {
                        ...
                        if (!thisElement.getPrinter().isCombinePrintjobs()) {
                            ...
                            Main.printers.print(getPrintableObject(thisElement), thisElement.getPrintername(), DocFlavor.SERVICE_FORMATTED.PRINTABLE);

                            pb.setValue(progressbar);
                            pb.setString("Drucke Nr. " + progressbar);
                            Main.debug("Drucke Nr. " + progressbar);
                            progressbar++;
                    ...
                    if (!preparedPrintJobs.isEmpty()) {
                        ...
                        for (ArrayList<PrintListElement> printListElements : collection) {
                            ...
                            try {
                                ...
                                Main.printers.print(encoded, printername, flavor);
                ...
                interrupted = true;
                Main.logger.debug("PrintProcessor interrupted!");
            }
        }
    }
